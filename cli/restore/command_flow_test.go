...
func TestNewCommandDryRunByID(t *testing.T) {
	...
	if _, err := session.DeleteSessions([]session.Session{selected}, session.Selector{ID: selected.SessionID}, session.DeleteOptions{
		DryRun:       false,
		Confirm:      true,
		Yes:          true,
		Hard:         false,
		...
		t.Fatalf("prepare trashed session: %v", err)
	}
	...
	if err := cmd.Execute(); err != nil {
		t.Fatalf("restore dry-run execute: %v stderr=%q", err, stderr.String())
	}

	if !strings.Contains(stdout.String(), "simulation=true") {
		t.Fatalf("expected dry-run summary, got: %q", stdout.String())
	}
}

func TestNewCommandRejectsBatchIDAndSelector(t *testing.T) {
	cmd := NewCommand(
	...
	cmd.SetOut(&stdout)
	cmd.SetErr(&stderr)
	cmd.SetArgs([]string{"--batch-id", "b-test", "--id", "deadbeef", "--dry-run"})
