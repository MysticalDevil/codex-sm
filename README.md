...
```bash
GOEXPERIMENT=jsonv2 go install github.com/MysticalDevil/codexsm@v0.3.8
```

Or with `mise`:

```bash
GOEXPERIMENT=jsonv2 mise install go:github.com/MysticalDevil/codexsm@v0.3.8
```
...
```bash
GOEXPERIMENT=jsonv2 go build -ldflags="-X main.version=0.3.8" -o codexsm .
```
